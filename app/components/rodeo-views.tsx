...
const moreItems = [
  {
    ...
    icon: Calendar,
    title: "Schedule",
    subtitle: "Upcoming rodeos, daysheets, and event details"
  },
  {
    id: "favorites",
    icon: Users,
...
export function MoreView({
  ...
  setScheduleDateRange: (range: DateRange) => void;
  onOpenScheduleRodeo: (rodeo: RodeoRow) => void;
  onLoadMoreSchedule: () => void;
  ...
  if (section === "schedule") {
    return (
      <SubViewShell title="Schedule" onBack={() => setSection("menu")}>
        <ScheduleView
          rows={scheduleRows}
          state={scheduleState}
          dateRange={scheduleDateRange}
          setDateRange={setScheduleDateRange}
          onOpenRodeo={onOpenScheduleRodeo}
          onLoadMore={onLoadMoreSchedule}
        />
      </SubViewShell>
    );
  }
...
export function AthleteDetailPane({
  ...
  useEffect(() => {
    ...
    setSelectedEvent((current) => {
      if (current && availableEvents.includes(current)) return current;
      const normalizedPreferredEvent = teamRopingResultEvent(preferredEvent ?? "");
      if (normalizedPreferredEvent && availableEvents.includes(normalizedPreferredEvent)) return normalizedPreferredEvent;
      return defaultAthleteBioEvent(bio, availableEvents);
    });
  }, [availableEvents, bio, preferredEvent]);
